...
BEFORE
    ...
    UPDATE OF END_DATE OR
    UPDATE OF DISCOUNT_ID ON RESERVATIONS
FOR EACH ROW
DECLARE
...
BEGIN
    ...
    IF(:NEW.DISCOUNT_ID IS NULL) THEN
        DISCOUNT := 0;
    ELSE
        SELECT DISCOUNT_VALUE INTO DISCOUNT FROM DISCOUNTS WHERE DISCOUNT_ID = :NEW.DISCOUNT_ID;
    END IF;
    :NEW.ESTIMATED_COST := DAILY_ROOM_COST * (:NEW.END_DATE - :NEW.START_DATE) * ((100 - DISCOUNT) / 100);
...
BEGIN
    ...
    UPDATE SECTORS
        SET SECTOR_SIZE = NEW_SECTOR_SIZE+1
